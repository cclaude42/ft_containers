...
# endif
	...
	{
		size_type		off = position - this->begin();
		// std::cout << position.getPtr() << " - " << this->begin().getPtr() << " = " << off << std::endl;
		...
		// std::cout << "Constructing from " << _size << " to " << _size + n - 1 << std::endl;
		for (size_type i = 0 ; i < n ; i++)
			_alloc.construct(_vct + _size + i, val);
		// std::cout << "Copying " << n << " down from " << off << " to " << _size - 1 << std::endl;
		for (int i = _size - 1 ; i >= 0 && i >= (int)off ; i--)
			_vct[i + n] = _vct[i];
		// std::cout << "Filling in from " << off << " to " << off + n - 1 << std::endl;
